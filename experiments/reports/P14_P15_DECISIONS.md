...
- 没有 cluster-level timing 证据表明 SMU 会进入系统 critical path；P8 的最终
  STOP 证据是完整 SV elaboration、proc/opt/memory_collect 通过后在 flatten
  阶段因资源/OOM 退出 137，因此 cluster timing unavailable。
...
CSV：`experiments/parsed/p16/p16_hardware_results.csv`。Cluster area/timing、
physical area、signoff timing、power、energy 均 unavailable。
