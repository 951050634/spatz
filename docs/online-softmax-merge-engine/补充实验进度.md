# Online Softmax Merge 补充实验进度

> 审计日志：保留所有 checkpoint、失败和哈希。面向读者的当前结论见
> [实验设计与结果.md](实验设计与结果.md)，不要从单个历史 checkpoint 推断当前状态。

本文档保存补充实验的版本化进度和审计记录。详细原始 artifact 保留在 Git 外部的
`work-online-merge-*` 目录中，仓库只记录小型 manifest、结构化结果和哈希。
...
| 10 | Scalar-only A1 | P1 | validation_complete | dirty three-anchor run; clean publication rerun pending |
